    ...
    if ($password == $cpassword) {
            if (strlen($p) > 5) {
                ...
                if (!$query_run->num_rows > 0) {
                    $query = "SELECT * FROM agency WHERE username = '$username' AND email = '$email'";
                    $query_run = mysqli_query($conn, $query);

                    if(!$query_run->num_rows > 0){
                        $query2 = "INSERT INTO agency(username,email,`password`,category_id)
                        VALUES ('$username', '$email', '$password', '$category_id')";
                        $query_run2 = mysqli_query($conn, $query2);

                        if ($query_run2) {
                            $_SESSION['agencyname'] = $_POST['username'];
                            echo "<script> alert('Regestration Successfull.');
                            window.location.href='agency_login.php';
                            </script>";
		...
		<div class="col-12 col-md-7 col-lg-12 auth-main-col text-center p-5">
			<div class="d-flex flex-column align-content-center">
				<div class="app-auth-body mx-auto">
					<h2 class="auth-heading text-center mb-3">Sign Up</h2>
					...
					<div class="auth-form-container text-start mx-auto">
						<form class="auth-form auth-signup-form" method="post" action="">
							...
							<div class="email mb-3">
								<label class="sr-only" for="signup-email" style="padding-bottom:10px;">Agency Category</label>
							...
							<div class="email mb-3">
								<label class="sr-only" for="signup-email" style="padding-bottom:10px;">Agency Username</label>
								<input id="signup-name" name="username" type="text" class="form-control signup-name"
									placeholder="Full name" required>
							</div>
							<div class="email mb-3">
								<label class="sr-only" for="signup-email" style="padding-bottom:10px;">Email</label>
								<input id="signup-email" name="email" type="email" class="form-control form-cup-email"
									placeholder="Email" required>
							</div>
							<div class="password mb-3">
								<label class="sr-only" for="signup-password" style="padding-bottom:10px;">Password</label>
								<input id="signup-password" name="password" type="password"
									class="form-control signup-password" placeholder="Create a password" required>
							</div>
							<div class="password mb-3">
								<label class="sr-only" for="signup-password" style="padding-bottom:10px;">Confirm Password</label>
						...
						<div class="auth-option text-center pt-5">Already have an account? <a class="text-link"
								href="agency_login.php">Log in</a></div>
					</div>
					<!--//auth-form-container-->
