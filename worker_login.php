...
if (isset($_POST['submit'])) {
	...
	if ($result->num_rows > 0) {
        ...
        $sql = "SELECT * FROM workers WHERE `password`='$password'";
        $result = mysqli_query($conn, $sql);
        ...
        if ($result->num_rows > 0) {
            ...
            if ($result->num_rows > 0) {
                $_SESSION['workername'] = $_POST['username'];

                $sql = "INSERT INTO latest_users(`image`, `name`, `role`) VALUES ((SELECT `worker_img` FROM workers WHERE username='$username'), '$username', 'Worker')";
                $result = mysqli_query($conn, $sql);
                if($result){
					 #setting active status to 1
					 $sql = "UPDATE workers SET `status`=1 WHERE username='$username'";
					 $result = mysqli_query($conn, $sql);
					 if($result){
                    	header("Location: worker_home.php");
					 }
				...
				<div class="app-auth-body mx-auto">
					...
					<div class="auth-form-container text-start">
						...
						<div class="auth-option text-center pt-5">No Account? Sign up <a class="text-link"
								href="worker_signup.php">here</a>.</div>
					</div>
					<!--//auth-form-container-->
