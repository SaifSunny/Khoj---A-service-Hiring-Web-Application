...
if (isset($_POST['submit'])) {
	...
	if ($result->num_rows > 0) {
        ...
        $sql = "SELECT * FROM agency WHERE `password`='$password'";
        $result = mysqli_query($conn, $sql);
        ...
        if ($result->num_rows > 0) {
            ...
            if ($result->num_rows > 0) {
                ...
                $sql = "INSERT INTO latest_users(`image`, `name`, `role`) VALUES ((SELECT `agency_img` FROM agency WHERE username='$username'), '$username', 'Agency')";
                $result = mysqli_query($conn, $sql);
                if($result){
					 #setting active status to 1
					 $sql = "UPDATE agency SET `status`=1 WHERE username='$username'";
					 $result = mysqli_query($conn, $sql);
					 if($result){
						header("Location: agency_home.php");
					 }
				...
				<div class="app-auth-body mx-auto">
					...
					<div class="auth-form-container text-start">
						...
						<div class="auth-option text-center pt-5">No Account? Sign up <a class="text-link"
								href="agency_signup.php">here</a>.</div>
					</div>
					<!--//auth-form-container-->
